import type { GetStaticProps, NextPage } from 'next';
import Head from 'next/head';
import Link from 'next/link';
import { CATEGORIES, getEntriesByCategory } from '@/lib/entries';
import CategoryIcon from '@/components/ui/CategoryIcon';

interface CategoryIndexProps {
  categories: { name: string; count: number }[];
}

const CATEGORY_FRAMES: Record<string, string> = {
  Expletive: 'Pure emotional discharge. The expletive exists to carry force, not meaning — and the best fictional ones land immediately, before a reader even knows what they signify.',
  Insult:    'Targeted and personal. Fictional insults reveal social hierarchies: who can be degraded, in what terms, and what the choice of words says about the insulter as much as the target.',
  Euphemism: 'The polite route around something impolite. Euphemisms are often the most culturally revealing words in a fiction — they show what a society acknowledges but can\'t bring itself to say directly.',
  Curse:     'Invocations with intent. Curses carry structure and weight, and usually a mythology behind them. The fictional curse tells you what a world fears.',
  Oath:      'The solemn register. Oaths are what characters swear by, which tells you what they hold sacred. A fiction\'s oaths are a shortcut to its cosmology.',
  Slang:     'The informal layer — words that signal in-group membership and generational identity. Slang ages the fastest and dates a work most precisely.',
};

const categoryColors: Record<string, string> = {
  Expletive: '#C2410C',
  Insult:    '#7E22CE',
  Euphemism: '#0369A1',
  Curse:     '#B91C1C',
  Oath:      '#3730A3',
  Slang:     '#0F766E',
};

const CategoryIndexPage: NextPage<CategoryIndexProps> = ({ categories }) => {
  return (
    <>
      <Head>
        <title>Browse by Category &mdash; HolyShirtBalls</title>
        <meta name="description" content="Explore fictional profanity by type — Expletive, Insult, Euphemism, Curse, Oath, and Slang." />
      </Head>

      <div className="max-w-6xl mx-auto px-4 py-10">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-2 text-sm mb-8" aria-label="Breadcrumb">
          <Link href="/" className="text-ink-400 hover:text-brand-coral transition-colors duration-150">
            Home
          </Link>
          <span className="text-ink-200">/</span>
          <span className="text-ink-700">Category</span>
        </nav>

        <header className="pb-10 mb-10 border-b border-ink-100 max-w-3xl">
          <p className="eyebrow mb-4">Browse by</p>
          <h1
            className="font-display font-extrabold leading-none mb-4 text-ink-900"
            style={{ fontSize: 'clamp(2.5rem, 7vw, 4.5rem)', letterSpacing: '-0.02em' }}
          >
            Category
          </h1>
          <p className="leading-relaxed text-ink-800" style={{ fontSize: '1.125rem', maxWidth: '52ch' }}>
            Six categories. Each one describes a different function that invented language can serve.
          </p>
        </header>

        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {categories.map(({ name, count }) => (
            <Link
              key={name}
              href={`/category/${name.toLowerCase()}`}
              className="card block p-6"
            >
              {/* Category accent color is intentional semantic distinction — not a token */}
              <div
                className="flex items-center gap-2 mb-3"
                style={{ color: categoryColors[name] ?? '#1A1210' }}
              >
                <CategoryIcon category={name} size={20} />
                <h2 className="font-display font-extrabold text-xl">{name}</h2>
              </div>
              <p className="text-sm leading-relaxed mb-4 text-ink-600">
                {CATEGORY_FRAMES[name]}
              </p>
              <p className="text-xs font-semibold text-ink-400">
                {count} {count === 1 ? 'entry' : 'entries'}
              </p>
            </Link>
          ))}
        </div>
      </div>
    </>
  );
};

export const getStaticProps: GetStaticProps<CategoryIndexProps> = () => {
  const categories = CATEGORIES.map((name) => ({
    name,
    count: getEntriesByCategory(name).length,
  }));
  return { props: { categories } };
};

export default CategoryIndexPage;
